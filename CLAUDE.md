...
## 이어서 작업할 때 — `docs/NEXT.md` 먼저 읽어라

작업 지시가 **"다음 할 작업 뭐지?" · "이어서 하자" · "어디까지 했지?"** 처럼 **현재 상태나 다음 순서를 묻는 것**이면
다른 문서를 뒤지기 전에 [`docs/NEXT.md`](docs/NEXT.md)를 읽는다. 착수 가능한 작업(우선순위 순) · 외부 회신 대기
항목 · 🔴 착수 금지 항목 · 최근 결정 로그가 거기 모여 있다. 작업 환경(PC)이 바뀌어도 이 파일이 인수인계 지점이다.

- `docs/meetings/*-작업플랜.md`의 완료 표시(✅)와 실제 트리가 어긋난 구간이 있다 → `docs/NEXT.md` §1이 그 차이를 기록한다.
  **어긋나면 `docs/NEXT.md`가 현재 상태**, 작업플랜은 원래 의도로 읽는다.
- 작업을 마치면 `docs/NEXT.md`를 갱신한다 (완료 항목 삭제 · 새 항목 추가 · 마지막 갱신일·커밋 갱신).
  단, **결정 내용 자체는 거기 쓰지 않는다** — `context/`·`guardrails/`에 심고 링크만 남긴다 (아래 절대규칙 1).
