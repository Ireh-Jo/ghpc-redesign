...
- [ ] 그라데이션 사용은 헤로 오버레이 스크림만 — 메인 영상 오버레이(다크) + **서브 히어로 다크 그라데이션**(2026-09-18: 웜 화이트 스크림 폐기, 텍스트 쪽만 누르고 반대편은 `to-transparent`). 장식용 컬러 그라데이션은 여전히 금지
...
- [ ] 모바일 햄버거 메뉴에서 "새가족"은 **맨 아래 + highlight 색** (2026-08-12 결정 — 최상단 배치 안 함)
- [ ] 데스크탑 메가메뉴는 **hover한 대메뉴 하나만** 펼침 (5개 동시 노출 금지)
- [ ] 대메뉴는 **링크가 아님** — 클릭 불가(`cursor-default`, hover 시 색 변화 없음). 패널 안 하위 링크만
      hover 시 accent로 반응해 "무엇이 눌리는지"가 색 대비로 드러남
...
- [ ] 메가메뉴·모바일 `새가족` 화면에 "새가족 안내 바로가기 →" CTA (대메뉴 클릭 불가 정책의 도달성 보완)
