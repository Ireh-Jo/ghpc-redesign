...
## 2026-09-18 — 첫 탭은 페이지 맨 위로

첫 탭을 누르면 첫 섹션 앵커가 아니라 **페이지 최상단**으로 스크롤한다. 앵커로 가면 sticky 헤더 아래로
밀려 **상단 배너를 볼 일이 없다**는 사용자 지적(2026-09-18) 때문이다.

- `href`는 `#<첫 섹션 id>` 그대로 둔다 — JS가 없으면 원래대로 섹션으로 이동한다
- 클릭 시 `preventDefault` → `scrollTo({top:0})` → `history.replaceState`로 해시를 지운다
