...
- **장소 필터** — `전체 장소` + **장소 마스터 14곳 전부**를 분류별(`optgroup`)로 보여준다.
  예약이 있는 곳만 추리지 않는다 — 달력의 목적이 빈 시간 확인이라 **"예약 0건"이 곧 필요한 답**이고,
  신청 폼과 같은 목록(`lib/rooms.ts`)을 써야 "필터엔 없는데 폼엔 있는" 혼란이 없다 (2026-09-19 수정)
- 예약이 없는 장소를 고르면 "이용 가능 시간 전체가 열려 있습니다"로 안내한다 (빈 화면 방치 금지)
