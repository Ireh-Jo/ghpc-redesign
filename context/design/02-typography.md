...
## 서체 추가 대기 — AG 최정호 스크린 (2026-09-19 디자인팀 요청)

**교육 메뉴**에 `AG 최정호 스크린`을 추가해 달라는 요청이 왔다. 안그라픽스의 **유료 폰트**로,
파일과 웹 라이선스가 확보되기 전에는 적용할 수 없다 (CDN·구글폰트에 없고 임의 변환·자가 호스팅은 위반).

> DECISION NEEDED: 웹폰트 파일(woff2) · 웹 라이선스 범위(도메인·PV) · 자수 서브셋 · **사용 범위**.
> 요청 항목과 드롭인 절차는 `public/fonts/README.md`에 정리해 뒀다.

**권장 사용 범위:** 교육 페이지의 **제목(h2·h3)만.** 본문은 Pretendard를 유지한다 —
한글 디스플레이 서체는 본문 가독성이 떨어지고, 한글 웹폰트 용량이 LCP를 직접 깎는다
(`guardrails/05-performance.md`). 제목만이면 서브셋이 작아 체감 비용이 거의 없다.

**한 페이지에 서체 2종이 되는 점**은 기존 원칙(Pretendard 단독)의 예외다. 적용 시 이 문서에
"교육 페이지 제목 한정"을 명시하고, 다른 페이지로 번지지 않게 한다.
