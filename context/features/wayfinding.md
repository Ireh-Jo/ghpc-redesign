...
> ~~지상층 표시 70% 축소~~ — **2026-07-01 폐기(비잠금 재분류).** 프로토타입의 `width:100%` 렌더링 방식에서
> 지상층(좌표계가 더 작은 841×595)이 지하층보다 확대돼 보이던 것을 상쇄하려던 값이었음. React 이전 시
> 지도를 `preserveAspectRatio="xMidYMid meet"`로 박스에 꽉 채우는 방식(contain-fit)으로 바꾸면서 이 보정의
> 전제가 사라졌고, 오히려 지상층이 지하층보다 작아 보이는 반대 문제를 만들었음 → 제거(`lib/wayfind/floors.ts`
> `display` 필드 삭제). 두 층 세트의 도면 가로세로 비율 차이(지상 1.41:1 vs 지하 1.76:1)로 인한 자연스러운
> 여백 차이는 남지만(왜곡 없이 완전히 일치시키는 건 좌표 정합을 깨뜰 수 있어 불가), 체감 차이는 크게 줄었음.
> 이 항목은 원래도 라우팅 데이터/알고리즘이 아니라 프리젠테이션(화면 표시 배율) 문제였어서, "잠긴 항목"이
> 아니라 아래 "비잠금" 절에 있었어야 했음 — 분류 오류였음.
