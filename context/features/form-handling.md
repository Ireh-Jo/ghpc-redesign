...
> **2026-08-23**: 신청 폼 4종은 현행 사이트(ghpc.or.kr)의 같은 폼을 **필드 단위로 그대로** 옮겼다
> (사용자 지시). 바뀐 것은 입력 방식뿐 — 오전/오후 + 시 + 분 select 3단 → `type="time"`.
> 폼마다 JSX를 복붙하지 않고 **폼 정의(`lib/forms/definitions.ts`) + 공통 렌더러(`ApplyForm`)** 구조로 뒀다.
> 필드가 바뀌면 정의 배열과 `lib/schemas/*`만 고친다.
