...
/admin/posts            소식 (영상/텍스트/교우/교단) + 공지사항
/admin/banners          메인 배너 (2026-09-16 추가 — 메인 시안의 중앙 배너)
/admin/staff            섬기는 사람들
...
## 2026-09-16 추가 — 메인 관리 영역 2종

메인 시안을 반영하면서 **관리자가 넣고 빼는 영역은 공지사항·배너 둘**로 확정됐다(사용자 지시).
지금은 `lib/notices.ts`·`lib/main-banners.ts` 파일이 출처라 **미디어팀이 못 바꾼다**(코드 수정+배포 필요).
어드민 착수 시 이 둘이 **1순위 화면**이다 — 매주(공지)·행사마다(배너) 바뀌는 것들이라 손이 제일 많이 간다.

선행 조건은 Supabase 프로젝트 + `.env.local` (`context/ops/supabase-setup.md`), 그리고
`context/03-data-model.md`의 미결 2건(공지 카테고리 · 배너 테이블) 확정이다.

**2026-09-16 사용자 결정: 착수는 나중에 한 번에.** 단계별 실행 순서·검증 방법은
[`docs/어드민-착수-플랜.md`](../../docs/어드민-착수-플랜.md)에 정리돼 있다 (이 문서는 결정·스펙, 그쪽은 실행 순서).
