...
composes:
  ...
  sections:                                 # 스크롤 순서대로
    ...
    - { name: values, component: inline }                          # 공동체 가치 3 — 전용 컴포넌트 없음(재검토 중)
    - { name: newcomer, component: content/welcome-cta }          # 새가족 환영 임팩트
    - { name: live-visit, component: [inline, content/map-embed] } # LIVE(인라인) + 오시는 길(MapEmbed) 2컬럼
depends-on:
...
## 현재 구현 상태 (컴포넌트 분리 완료 — 2026-07-01)

> `app/(site)/page.tsx`는 이제 content 컴포넌트 조립 + 데이터 상수 전달만 담당.
> 콘텐츠·이미지는 여전히 placeholder (unsplash). 영상은 디자인팀 제작 전까지 정지 이미지.

| 섹션 | 상태 | 비고 |
|---|---|---|
| 헤로 | ✅ `content/HeroVideo` | placeholder 이미지 + 그라데이션 오버레이 |
| 이번 주 말씀 | ✅ `content/SermonCard` | 메인 영상 + 지난 설교 리스트 |
| 표어 배너 | ✅ `content/CampaignBanner` | 다크 + solid 오버레이 |
| 공동체 가치 3 | 인라인 유지 | gap-px 보더 그리드. 전용 컴포넌트 없음 — 유지/통합 결정 전까지(아래 결정 필요) |
| 새가족 환영 | ✅ `content/WelcomeCTA` | 다크 임팩트, `/care`에서도 재사용 예정 |
| LIVE + 오시는 길 | LIVE는 인라인, 오시는 길은 ✅ `content/MapEmbed` | **건물 내 길찾기**는 `/intro#directions`의 `interactive/FloorMap`으로 진입(`context/features/wayfinding.md`) |
