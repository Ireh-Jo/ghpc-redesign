...
**두 가지 모드:**
- **본 개발 모드** (디폴트) — Next.js + Supabase 운영 코드 작업
- **시안 모드** — 메시지에 "시안 모드" 마커 포함 시 활성. 정적 HTML 시안 (Tailwind CDN). 본 개발과 격리. § [3-F](#3-일상-워크플로--시나리오-6개) 참조.
...
### F. 시안 작업 (디자이너 협업·내부 시연용)

> "시안 모드. 03-worship.html 만들어줘. 메인 시안 토큰 그대로."

**전제:** 메시지에 **"시안 모드"** 마커가 있으면 Claude가 별도 컨텍스트로 전환.

Claude가 하는 것:
1. [CLAUDE.md](CLAUDE.md)의 모드 분기에서 "시안 모드" 감지
2. `prototypes/_archive/context/*` (디자인시스템·IA·콘텐츠·레퍼런스) + `prototypes/_archive/guardrails/*` 로드
3. `prototypes/designs/0X-{이름}.html`에 단일 HTML 생성 (Tailwind CDN, 빌드 도구 없음)
4. **본 개발 `context/`·`guardrails/` 는 건드리지 않음**

**핵심 격리 원칙:**
- 시안 모드와 본 개발 모드는 **양방향 격리.** 한쪽 작업이 다른 쪽 결정사항을 자동으로 바꾸지 않음.
- 시안에서 새로 결정 잠긴 항목 → 본 개발에 반영하려면 별도로 "결정 잠금" 작업 요청 (시나리오 D).
- 시안 HTML을 본 개발 코드로 직접 복붙 금지 — 디자인 토큰·접근성·성능 재검증 거쳐야 함.

**산출물 위치:**
- 시안 HTML: `prototypes/designs/01-main.html` ~ `05-activity.html`
- 더블클릭으로 브라우저에서 바로 열림

**현재 시안 상태:**
- `01-main.html` 1개 작성됨 (`prototypes/designs/index.html` → 추후 리네이밍 검토)
- 02~05 미작성. 안건 일정상 디자이너 협업 방식 결정 후 진행 또는 Figma로 전환
...
**시안 모드 패턴:**
- 첫 단어에 **"시안 모드"** 명시. 예: "시안 모드: 04-care.html 만들자."
- 시안 모드에선 본 개발 디렉토리 안 건드림. Tailwind CDN 단일 HTML.
...
- **02-intro.html ~ 05-activity.html 추가 제작** — 디자이너 협업 방식 결정 전 내부 시연·비교용
- **prototypes/designs/index.html → 01-main.html 리네이밍** — 파일명 컨벤션 통일
- **시안 일관성 교차검증** — 5개 시안이 모두 같은 토큰 사용하는지 ([prototypes/_archive/guardrails/01-consistency-checklist.md](prototypes/_archive/guardrails/01-consistency-checklist.md))
...
- "Claude가 시안 모드를 본 개발로 착각하거나 반대" → 메시지 첫 줄에 모드 명시 ("시안 모드:" 또는 "본 개발:")
